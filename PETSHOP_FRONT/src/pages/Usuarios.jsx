...
export default function Usuarios() {
  ...
  async function handleDelete(id_usuario) {
    try {
      logger.info('Usuarios', `Cambiando estado para usuario ID: ${id_usuario}`);
      
      const deps = await userService.getDependencies(id_usuario);
      
      if (deps.estado_actual === 'activo') {
        const mensaje = `El usuario tiene ${deps.dependencias?.clientes || 0} cliente(s) y ${deps.dependencias?.cierres_caja || 0} cierre(s) de caja asociados.\n\n¿Desea desactivarlo?`;
        if (!confirm(mensaje)) return;
      } else {
        if (!confirm('¿Desea activar este usuario?')) return;
      }
      
      await userService.toggleStatus(id_usuario);
      logger.success('Usuarios', 'Estado cambiado exitosamente');
      toast.success(deps.estado_actual === 'activo' ? 'Usuario desactivado' : 'Usuario activado');
      loadUsuarios();
    } catch (error) {
      logger.error('Usuarios', 'Error al cambiar estado', error);
      toast.error(getErrorMessage(error));
    }
  }
              ...
              {usuarios.length === 0 ? (
                ...
                usuarios.map((u) => (
                  <tr key={u.id_usuario}>
                    ...
                    <td>
                      <button 
                        className={`btn btn-sm ${Number(u.activado) === 1 || u.activado === true ? 'btn-danger' : 'btn-success'}`}
                        onClick={() => handleDelete(u.id_usuario)}
                        style={{ minWidth: 70 }}
                      >
                        {Number(u.activado) === 1 || u.activado === true ? 'Inactivar' : 'Activar'}
                      </button>
                    </td>
                    <td>
